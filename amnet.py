...
class PredictionResult():
    ...
    def write_stdout(self):
        ...
        for i, (image, prediction, target) in enumerate(zip(self.image_names, self.predictions, self.targets)):
            ...
            print(str(i)+'\t'+image + (' '*(max_len-len(image)+5)) + str(round(prediction,3)) + '  \t' + str(round(target.item() if hasattr(target, 'item') else target, 3)) )
...
class AMNet:
    ...
    def load_dataset(self, split='train_1', train=True, batch_size=512, dataset_root='datasets/lamem/',
        ...
        # Use different dataset class for memcat_vehicle
        if self.hps.dataset_name == 'memcat_vehicle':
            from image_dataset import MemCatVehicleDataset
            dataset = MemCatVehicleDataset(os.path.join(dataset_root, self.hps.images_dir),
                                        split_root = os.path.join(dataset_root, self.hps.splits_dir),
                                        split=split,
                                        transform=self.train_transform if train else self.test_transform)
        else:
            from lamem2 import LaMem2
    ...
    def train_epoch(self, epoch, train_loader):
        ...
        if self.hps.front_end_cnn == 'ResNet50FC':
            # Unfreeze later layers for fine-tuning
            for name, param in self.model.core_cnn.named_parameters():
                if 'layer3' in name or 'layer4' in name:  # Unfreeze later layers
                    param.requires_grad = True
                else:  # Freeze early layers
        ...
        for batch_idx, (data, target, _) in enumerate(train_loader):
            ...
            self.logger.write(train=True, epoch=epoch, epoch_samples=total_samples,
                              ...
                              loss=loss.cpu().item(), lr=params.lr)
        ...
        self.logger.write(train=True, epoch=epoch, epoch_samples=len(train_loader.dataset),
                          sample=(batch_idx * len(data)),
                          loss=loss.cpu().item(), lr=params.lr, src=rc)
...
# Evaluation
    def eval_model(self, test_loader):
        ...
        for data, target, _ in test_loader:
            ...
            test_att_loss += att_loss.cpu().item() if att_loss is not None else 0
            test_reg_loss += reg_loss.cpu().item()
            test_mem_loc_loss += mem_loc_loss.cpu().item()
